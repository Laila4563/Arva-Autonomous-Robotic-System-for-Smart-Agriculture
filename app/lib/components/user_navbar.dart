...
class UserNavbar extends StatefulWidget {
  ...
  static _UserNavbarState? of(BuildContext context) =>
      context.findAncestorStateOfType<_UserNavbarState>();
...
class _UserNavbarState extends State<UserNavbar> {
  ...
  void loginUser() {
    setState(() {
      isLoggedIn = true;
      _currentIndex = 1; // Move to Dashboard after login
    });
  }
  ...
  void logoutUser() {
    setState(() {
      isLoggedIn = false;
      _currentIndex = 0; // Return to Landing Page
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: !isLoggedIn 
        ? const SizedBox.shrink() // Hide navbar completely if not logged in
        : ClipRRect(
  ...
  int _getNavbarIndex(int stackIndex) => stackIndex >= 4 ? 4 : stackIndex;
      ...
      child: Column(
        children: [
          Container(
            ...
            decoration: BoxDecoration(shape: BoxShape.circle, color: sproutGreen.withValues(alpha: 0.1), border: Border.all(color: sproutGreen.withValues(alpha: 0.2))),
      ...
      icon: Stack(
        ...
        children: [
          if (isActive)
            Container(
              width: 40, height: 40,
              decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [BoxShadow(color: sproutGreen.withValues(alpha: 0.4), blurRadius: 15, spreadRadius: 1)]),
            ),
