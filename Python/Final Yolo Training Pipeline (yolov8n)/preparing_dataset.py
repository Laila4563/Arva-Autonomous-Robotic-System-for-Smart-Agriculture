...
def clean_working_directory():
    """Clears and recreates the working directory for a clean run."""
...
class EnhancedDataBalancer:
    ...
    def _load_class_names(self):
        ...
        try:
            ...
            # Update paths in the config to point to the new local folder
    ...
    def _copy_original_data(self):
        for dtype in self.data_types:
            src_images = f"{self.root}/{dtype}/images"
            if os.path.exists(src_images):
                for img_file in tqdm(glob(f"{src_images}/*"), desc=f"Copying {dtype} images"):
                    shutil.copy(img_file, f"{self.output_dir}/{dtype}/images/")

            src_labels = f"{self.root}/{dtype}/labels"
            if os.path.exists(src_labels):
                for label_file in tqdm(glob(f"{src_labels}/*"), desc=f"Copying {dtype} labels"):
                    shutil.copy(label_file, f"{self.output_dir}/{dtype}/labels/")
